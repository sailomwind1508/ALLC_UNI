...
namespace AllCashUFormsApp.Model
{
    ...
    public partial class DB_ALL_CASH_UNIEntities
    {
        private Dictionary<string, string> ConvertToSqlType = new Dictionary<string, string>();

        public DB_ALL_CASH_UNIEntities(int commandTimeout) : base("name=DB_ALL_CASH_UNIEntities")
        {
        ...
        private DataTable ConvertToDataTable<T>(IEnumerable<T> list, string tableName)
        {
            ...
            var props = typeof(T).GetProperties(BindingFlags.Public | BindingFlags.Instance)
                                       ...
                                       .Where(y => y.GetCustomAttributesData().ToList().Any(z => z.GetType().Name == "DataMemberAttribute"))
                                       .ToList();

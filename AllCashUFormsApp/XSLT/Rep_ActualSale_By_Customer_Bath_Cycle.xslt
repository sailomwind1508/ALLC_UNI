<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:key name="kRowByName" match="Rep_ActualSale_By_Customer_Bath_XSLT_Cycle" use="DocKey"/>
...
grouping-separator="'" digit="#" zero-digit="0" decimal-separator="."/>
	...
	<xsl:template match="NewDataSet">
		<html>
			...
			<table>
				<Worksheet Name="รายวัน">
					<TR>
						<TD class="stdPageHdr" colspan="7">
							<xsl:value-of select="Rep_ActualSale_By_Customer_Bath_XSLT_Cycle/CompanyName" />
						</TD>
					</TR>
					<TR>
						<TD class="stdPageHdr" Colspan="7">
							รายงานยอดขายแยกตามลูกค้า(บาท)
						</TD>
					</TR>
					<TR>
						<TD Colspan="7"></TD>
					...
					<TR>
						...
						<TD colspan="1" class="SearchResultItem" style="text-align: center;">
							<xsl:value-of select="Rep_ActualSale_By_Customer_Bath_XSLT_Cycle/Year" />
						</TD>
						...
						<TD class="SearchResultItem" Colspan="4" style="text-align: center;">
							<xsl:value-of select="Rep_ActualSale_By_Customer_Bath_XSLT_Cycle/BranchName" />
						</TD>
                ...
                "//Rep_ActualSale_By_Customer_Bath_XSLT_Cycle
                [generate-id() = 
	...
	<xsl:template match="Rep_ActualSale_By_Customer_Bath_XSLT_Cycle">
		<tr>
	...
	<xsl:template match="Cycle" mode="row">
		...
		<td class="SearchResultItem" align="Right">
			<xsl:if test="$nRows[Cycle=current()]/Amount=0">
				-
			</xsl:if>
			<xsl:if test="$nRows[Cycle=current()]/Amount>0">
				<xsl:value-of select='format-number($nRows[Cycle=current()]/Amount,"###,###.00")'/>
			</xsl:if>
		</td>
	</xsl:template>
</xsl:stylesheet>

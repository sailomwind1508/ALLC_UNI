<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Rep_IV_Sales_XSLT">
		<xsl:text disable-output-escaping="yes">
    <![CDATA[<?mso-application progid="Excel.Sheet"?> ]]>    
    </xsl:text>
		<ss:Workbook>
			<ss:Worksheet ss:Name="รายงานภาษีขายอย่างย่อ">
				<ss:Table>
					<xsl:apply-templates select="Rep_IV_Sales_XSLT"/>
				</ss:Table>
			</ss:Worksheet>
		</ss:Workbook>
	</xsl:template>
				...
				<TABLE>
					<TR>
						<TD class="stdPageHdr" colspan="11">
							<xsl:value-of select="/NewDataSet/Rep_IV_Sales_XSLT/Company"/>
						</TD>
					</TR>
					<TR>
						<TD class="stdPageHdr" colspan="11">รายงานภาษีขายอย่างย่อ</TD>
					</TR>
					<TR>
						<TD class="SearchKey">ระหว่างวันที่</TD>
						<TD class="SearchValue" Colspan="1">
							<xsl:value-of select="/NewDataSet/Rep_IV_Sales_XSLT/StartDate"/>


						</TD>
						<TD class="SearchKey">ถึงวันที่</TD>
						<TD class="SearchValue" Colspan="1">
							<xsl:value-of select="/NewDataSet/Rep_IV_Sales_XSLT/EndDate"/>
					...
					<TR>
						...
						<TD class="SearchValue" ColSpan="2">
							<xsl:value-of select="/NewDataSet/Rep_IV_Sales_XSLT/BranchName"/>
						</TD>
					</TR>
					<TR>
						<TD class="SearchKey">วันที่พิมพ์รายงาน</TD>
						<TD class="SearchValue" Colspan="6">
							<xsl:value-of select="/NewDataSet/Rep_IV_Sales_XSLT/Period"/>
						</TD>
					</TR>
					<TR>
					...
					<xsl:for-each select="NewDataSet/Rep_IV_Sales_XSLT">
					...
					<TR Class="GroupFooter">
						...
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_IV_Sales_XSLT/Amount),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_IV_Sales_XSLT/VatAmt),"#,##0.00")'/>
						</TD>
					</TR >
				</TABLE>
			</BODY>

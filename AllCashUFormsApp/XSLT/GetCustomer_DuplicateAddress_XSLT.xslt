<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="proc_GetCustomer_DuplicateAddress">
		<xsl:text disable-output-escaping="yes">
    <![CDATA[<?mso-application progid="Excel.Sheet"?> ]]>    
    </xsl:text>
		<ss:Workbook>
			<ss:Worksheet ss:Name="รายงานที่อยู่ซ้ำ">
				<ss:Table>
					<xsl:apply-templates select="proc_GetCustomer_DuplicateAddress"/>
				</ss:Table>
			</ss:Worksheet>
		</ss:Workbook>
	</xsl:template>
	...
	<xsl:template match="/">
		<HTML>
			...
			<BODY class="content">
				<TABLE>
					<TR>
						<TD class="stdPageHdr" colspan="3">รายงานที่อยู่ซ้ำ</TD>
						<TR>
							<TD Colspan="2" align="right">ศูนย์ :</TD>
							<TD align="left">
								<xsl:value-of select="NewDataSet/proc_GetCustomer_DuplicateAddress/BranchName"/>
							</TD>
						</TR>
					</TR>
					...
					<xsl:for-each select="NewDataSet/proc_GetCustomer_DuplicateAddress">
						<TR>

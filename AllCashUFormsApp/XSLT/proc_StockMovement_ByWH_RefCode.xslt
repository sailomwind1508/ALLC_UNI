<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="proc_StockMovement_ByWH_RefCode_XSLT">
		<xsl:text disable-output-escaping="yes">
    <![CDATA[<?mso-application progid="Excel.Sheet"?> ]]>    
    </xsl:text>
		<ss:Workbook>
			<ss:Worksheet ss:Name="รายงานสรุปยอดเคลื่อนไหวสินค้า เรียงตามรหัสบัญชีหน่วยเล็ก">
				<ss:Table>
					<xsl:apply-templates select="proc_StockMovement_ByWH_RefCode_XSLT"/>
				</ss:Table>
			</ss:Worksheet>
		</ss:Workbook>
	</xsl:template>

	<xsl:key name="groupWHID" match="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT" use="WHID" />
				...
				<TABLE>
					<TR>
						<TD class="stdPageHdr" colspan="7">
							<xsl:value-of select="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT/CompanyName"/>
						</TD>
					</TR>
					<TR>
						<TD class="stdPageHdr" colspan="7">
							<xsl:value-of select="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT/BranchName"/>
						</TD>
					</TR>
					<TR>
						<TD class="stdPageHdr" colspan="7">
							รายงานสรุปยอดเคลื่อนไหวสินค้า เรียงตามรหัสบัญชีหน่วยเล็ก
						</TD>
					</TR>
					...
					<TR>
						...
						<TD align="left">
							<xsl:value-of select="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT/DateFr"/>
						</TD>
						...
						<TD align="left">
							<xsl:value-of select="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT/DateTo"/>
						</TD>
					</TR>


					<xsl:for-each select="NewDataSet/proc_StockMovement_ByWH_RefCode_XSLT[count(. | key('groupWHID', WHID)[1]) = 1]">
						<TR>
							<TD>
								คลัง
							</TD>

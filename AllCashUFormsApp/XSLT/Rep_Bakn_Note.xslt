    ...
    <![CDATA[<?mso-application progid="Excel.Sheet"?> ]]>    
		...
		<ss:Workbook>
			<ss:Worksheet ss:Name="รายงานส่งเงินประจำวัน">
				<ss:Table>
					<xsl:apply-templates select="Rep_Bakn_Note"/>
				</ss:Table>
			</ss:Worksheet>
		</ss:Workbook>
	</xsl:template>
				...
				<TABLE>
					...
					<TR>
						...
						<TD class="SearchValue" Colspan="2">
							<xsl:value-of select="NewDataSet/Rep_Bakn_Note/BrnName"/>
						</TD>
						<TD class="SearchKey">Date :</TD>
						<TD class="SearchValue" Colspan="2">
							<xsl:value-of select="NewDataSet/Rep_Bakn_Note/DocDate"/>
						</TD>
					</TR>
					<tr></tr>
					<TR>
					...
					<xsl:for-each select="NewDataSet/Rep_Bakn_Note">
					...
					<TR Class="GroupFooter">
						...
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Send),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Deposit),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Transfer),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Cheque),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/TotalSend),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Total),"#,##0.00")'/>
						</TD>
						<TD Class="subTotals" align="right">
							<xsl:value-of select='format-number(sum(/NewDataSet/Rep_Bakn_Note/Balance),"#,##0.00")'/>
						</TD>
					</TR >
					<TR></TR>
					<tr>

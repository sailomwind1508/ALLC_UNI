<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="Rep_Customer_List_By_WH_XSLT_Cycle">
    ...
    <![CDATA[<?mso-application progid="Excel.Sheet"?> ]]>    
		...
		<ss:Workbook>
			<ss:Worksheet ss:Name="รายงานสรุปจำนวนร้านค้าทั้งหมด(ตามคลังรถ)">
				<ss:Table>
					<xsl:apply-templates select="Rep_Customer_List_By_WH_XSLT_Cycle"/>
				</ss:Table>
			</ss:Worksheet>
		</ss:Workbook>
	</xsl:template>
	
  <xsl:key name="groupWHID" match="NewDataSet//Rep_Customer_List_By_WH_XSLT_Cycle" use="WHID" />
  <xsl:key name="groupEmpID" match="NewDataSet/Rep_Customer_List_By_WH_XSLT_Cycle" use="concat(WHID,' ',EmpID)" />
  <xsl:key name="groupSalAreaID" match="NewDataSet/Rep_Customer_List_By_WH_XSLT_Cycle" use="SalAreaID" />
				...
				<TABLE>
						...
						<TR>
							<TD class="stdPageHdr" align="left" colspan="14">
								<xsl:value-of select="NewDataSet/Rep_Customer_List_By_WH_XSLT_Cycle/CompanyName"/>
							</TD>
						</TR>
						...
						<TR>
							...
							<TD align="left">
								<xsl:value-of select="NewDataSet/Rep_Customer_List_By_WH_XSLT_Cycle/BranchName"/>
							</TD>
						</TR>
					
		<xsl:for-each select="NewDataSet/Rep_Customer_List_By_WH_XSLT_Cycle[count(. | key('groupWHID', WHID)[1]) = 1]">

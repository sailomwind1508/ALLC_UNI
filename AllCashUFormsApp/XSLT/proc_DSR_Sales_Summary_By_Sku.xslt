<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:key name="kRows" match="proc_DSR_Sales_Summary_By_Sku_XSLT" use="EmpName"/>
...
grouping-separator="'" digit="#" zero-digit="0" decimal-separator="."/>
	...
	<xsl:template match="NewDataSet">
		<html>
			...
			<table>
				<Worksheet Name="รายวัน">
					<TR>
						<TD class="stdPageHdr" colspan="13">
							<xsl:value-of select="proc_DSR_Sales_Summary_By_Sku_XSLT/CompanyName" />
						</TD>
					</TR>
					<TR>
						<TD class="stdPageHdr" Colspan="13">
					...
					<TR>
						...
						<TD class="SearchResultItem" Colspan="4" style="text-align: center;">
							<xsl:value-of select="proc_DSR_Sales_Summary_By_Sku_XSLT/BranchName" />
						</TD>
						...
						<TD colspan="2" class="SearchResultItem" style="text-align: center;">
							<xsl:value-of select="proc_DSR_Sales_Summary_By_Sku_XSLT/CurentDate" />
						</TD>
						<TD colspan="2" class="SearchResultItem" style="font-weight: bold;text-align: center;" align="Center">
							วันที่ :
						</TD>
						<TD colspan="4" class="SearchResultItem" style="text-align: center;">
							<xsl:value-of select="proc_DSR_Sales_Summary_By_Sku_XSLT/HDate" />
						</TD>
					</TR>
					<TR>
						<TD Colspan="13"></TD>
					...
					<thead>
						...
						<xsl:apply-templates select="$colIds">
							...
							<xsl:sort select="ProductSubGroupID"/>
                ...
                "//proc_DSR_Sales_Summary_By_Sku_XSLT
                [generate-id() = 
	...
	<xsl:template match="proc_DSR_Sales_Summary_By_Sku_XSLT">
		<tr>


			<xsl:choose>
				<xsl:when test='WHID="0Name" or EmpName="ฮ0" or EmpName="ฮ1" or EmpName="ฮ2"'>
	...
	<xsl:template match="ProductShortName" mode="row">
		...
		<td class="ResultItem" align="Right">
			...
			<xsl:choose>
				<xsl:when test="not(number($nRows[ProductShortName=current()]/TargetQty))">
					<xsl:value-of select='$nRows[ProductShortName=current()]/TargetQty'/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select='number($nRows[ProductShortName=current()]/TargetQty)'/>
				</xsl:otherwise>
			</xsl:choose>
		...
		<td class="ResultItem" align="Right">
			...
			<xsl:choose>
				<xsl:when test="not(number($nRows[ProductShortName=current()]/SalesQty))">
					<xsl:value-of select='$nRows[ProductShortName=current()]/SalesQty'/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select='number($nRows[ProductShortName=current()]/SalesQty)'/>
				</xsl:otherwise>
			</xsl:choose>
		...
		<td class="ResultItem" align="Right">
			...
			<xsl:choose>
				<xsl:when test="not(number($nRows[ProductShortName=current()]/Percentage))">
					<xsl:value-of select='$nRows[ProductShortName=current()]/Percentage'/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select='number($nRows[ProductShortName=current()]/Percentage)'/>
				</xsl:otherwise>
			</xsl:choose>
		...
		<td class="ResultItem" align="Right">
			...
			<xsl:choose>
				<xsl:when test="not(number($nRows[ProductShortName=current()]/Estimate))">
					<xsl:value-of select='$nRows[ProductShortName=current()]/Estimate'/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select='number($nRows[ProductShortName=current()]/Estimate)'/>
				</xsl:otherwise>
			</xsl:choose>
